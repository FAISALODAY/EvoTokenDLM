...
from utils.parser_helper import is_equiv, remove_boxed, last_boxed_only_string
...
def parse_svamp_answers(json_path=None, json_data=None):
    if json_path:
        with open(json_path, "r") as file:
            data = json.load(file)
    else:
    ...
    for item in data.get("generations", []):
        total_processed += 1
        ground_truth = float(item.get("ground_truth"))
        raw_generation = item.get("generations", "")
        question = item.get("question", "")
...
def aggregate_results(directory="."):
    ...
    for json_file in json_files:
        ...
        if setup_name:
            ...
            elif "svamp" in setup_name:
                (
                    correct,
                    processed,
                    detailed_results,
                    total_effective_tokens,
                ) = parse_svamp_answers(json_path=json_file)
...
def report_results(directory_base, k_softs, alpha_soft_masks):
    final_results = {}
    for k_soft in k_softs:
        for alpha_soft_mask in alpha_soft_masks:
            dir_name = f"{directory_base}_kSoft_{k_soft}_alphaSoftMask_{alpha_soft_mask}"
            setups, results = aggregate_results(directory=dir_name)
            for setup, res in setups.items():
                if setup in final_results:
                    final_results[setup] = max(final_results[setup], res["accuracy"])
                else:
                    final_results[setup] = res["accuracy"]
    
    # Header
    header_format = "{:<40} {:>12}"
    print(header_format.format("Setup (task_model_seqlen_diffusteps)", "Accuracy"))
    print("-" * 60)

    # Data rows
    row_format = "{:<40} {:>11.2f}%"
    for setup, accuracy in sorted(final_results.items()):
        print(row_format.format(setup, accuracy))

    print("=" * 60)

if __name__ == "__main__":
    directory_base = "outputs/eval_results"
    k_softs = [3]
    alpha_soft_masks = [0.9, 0.8, 0.7, 0.6, 0.5]
    report_results(directory_base, k_softs, alpha_soft_masks)
