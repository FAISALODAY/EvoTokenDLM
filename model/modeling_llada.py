# Modified from Fast-dLLM repos: https://github.com/NVlabs/Fast-dLLM
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if (soft_token is None) or (prob is None):
            ...
            soft_emb = self.transformer.wte(soft_token)  # [batch_size, seq_len, K, d_model]
            prob_safe = prob.clamp_min(0)
            denom = prob_safe.sum(dim=-1, keepdim=True).clamp_min(1e-12)  # [batch_size, seq_len, 1]
            prob_norm = prob_safe / denom                                  # [batch_size, seq_len, K]
            x = (prob_norm.unsqueeze(-1) * soft_emb).sum(dim=-2)      # [batch_size, seq_len, d_model]
